...
def test_svm_regression_epsilon_svm_recovers_true_coefficients(rng):
    n = 300
    x = rng.standard_normal((n, 2))
    true_beta = np.array([1.5, -0.8])
    y = x @ true_beta + rng.standard_normal(n) * 0.05

    result = svm_regression_dual(y, x, c=10.0, epsilon=0.05)
    assert np.allclose(result.beta, true_beta, atol=0.15)


def test_svm_regression_epsilon_svm_primal_matches_dual(rng):
    n = 100
    x = rng.standard_normal((n, 2))
    y = x @ np.array([1.5, -0.8]) + rng.standard_normal(n) * 0.05

    dual = svm_regression_dual(y, x, c=10.0, epsilon=0.05)
    primal = svm_regression_primal(y, x, c=10.0, epsilon=0.05)
    assert np.allclose(dual.beta, primal.beta, atol=0.1)
    assert np.isclose(dual.beta0, primal.beta0, atol=0.1)
